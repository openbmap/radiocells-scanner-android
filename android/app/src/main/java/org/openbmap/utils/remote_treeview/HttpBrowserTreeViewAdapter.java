...
public class HttpBrowserTreeViewAdapter extends RemoteFileTreeViewAdapter {
    /**
     * @param activity
     * @param treeStateManager
     * @param numberOfLevels
     */
    public HttpBrowserTreeViewAdapter(Activity activity,
    ...
    public void handleItemClick(final View view, final Object id) {
        ...
        if (remoteFile.isDirectory) {
            if (remoteFile.children != null) {
                ...
                BrowseHTTPTask task = new BrowseHTTPTask(this, remoteFile);
            ...
            if (!downloadsByUri.containsValue(remoteFile.getUri())) {
                ...
                if (file.exists() && (file.lastModified() >= remoteFile.timestamp)) {
                    ...
                    builder.setPositiveButton(getActivity().getString(R.string.yes), (dialog, whichButton) -> {
                        file.delete();
                        startDownload(remoteFile, file, view);
                    });

                    builder.setNegativeButton(getActivity().getString(R.string.no), (dialog, whichButton) -> {
                        // NOP
                    });
