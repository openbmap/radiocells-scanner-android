...
public class HostActivity extends TabActivity {
	...
	protected final void onDestroy() {
		...
		try {
			positionServiceManager.unbind();
		} catch (Throwable t) {
			Log.e(TAG, "Error unbinding gps position service");
			t.printStackTrace();
		}
	
		try {
			wirelessServiceManager.unbind();
		} catch (Throwable t) {
			Log.e(TAG, "Error unbinding wireless logger service");
			t.printStackTrace();
		}
	
		try {
			gpxLoggerServiceManager.unbind();
		} catch (Throwable t) {
			Log.e(TAG, "Error unbinding gps logger service");
			t.printStackTrace();
		}
